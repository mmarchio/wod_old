...
class DefaultController extends Controller
{
    ...
    protected static function generateCharacter($type)
    {
    ...
    //TODO: convert to static method
    protected static function getKeyLists($group)
    {
        $a = [];
    ...
    //TODO: convert to static method
    protected static function getDisciplineList($group, $traits)
    {
        $a = [];
        foreach ($group as $k => $v) {
            if ($k !== "total" && $k !== "target") {
    ...
    //TODO: convert to static method
    protected static function getBackgroundsList($group, $traits)
    {
        $a = [];
        foreach ($group as $k => $v) {
            if ($k !== "total" && $k !== "target") {
    ...
    //TODO: convert to static method
    protected static function generateGroup($group, $items, $traits)
    {
    ...
    //TODO: convert to static method
    protected static function setTargets(character_template $ct, $creation)
    {
    ...
    //TODO: convert to static method
    protected static function pst_attributes(character_template $ct, $creation)
    {
        $attributes = [
            "physical",
            "social",
...
=======
    ...
    protected static function findTrait($traits, int $category, int $subCategory, int $id = 0)
    {
        $a = [];
        $c = count($traits);
        for ($i=0; $i<$c; $i++) {
    ...
    protected static function freebies($ct, $creation)
>>>>>>> parent of a4fdbe1 (more code cleanup)
=======
     ...
     */
    public function freebiesAction(Request $request, $id)
>>>>>>> a4fdbe13853410f1ce64cd7deecc243dece193d2
    {
        $data = $this->getCharacterById($request, $id);
        $data->character->attributes = new \stdClass();
        $data->character->attributes->physical = $data->character->physical;
    ...
    protected static function setCharacter($type)
    {
        ...
        switch ($type) {
            case "kindred":
                ...
                self::buildTraitGroup($creation->attributes->physical, $character->attributes->physical);
                $character->attributes->social = new \stdClass();
                $character->attributes->social->total = 0;
                $character->attributes->social->target = 0;
                self::buildTraitGroup($creation->attributes->social, $character->attributes->social);
                $character->attributes->mental = new \stdClass();
                $character->attributes->mental->total = 0;
                $character->attributes->mental->target = 0;
                self::buildTraitGroup($creation->attributes->mental, $character->attributes->mental);
                ...
                self::buildTraitGroup($creation->abilities->talents, $character->abilities->talents);
                $character->abilities->skills = new \stdClass();
                $character->abilities->skills->total = 0;
                $character->abilities->skills->target = 0;
                self::buildTraitGroup($creation->abilities->skills, $character->abilities->skills);
                $character->abilities->knowledges = new \stdClass();
                $character->abilities->knowledges->total = 0;
                $character->abilities->knowledges->target = 0;
                self::buildTraitGroup($creation->abilities->knowledges, $character->abilities->knowledges);
                ...
                for ($i=0; $i<count($creation->clanDisciplines); $i++) {
                    $lookup = self::findTrait($creation->traits, 0, 0,$creation->clanDisciplines[$i]->getTrait());
                ...
                self::buildTraitGroup($creation->virtues, $character->advantages->virtues);
    ...
    protected static function buildTraitGroup($creationRules, $category)
    {
    ...
    protected static function setCreation($clan)
    {
        $creation = new \stdClass();
        $creation->points = $this->getDoctrine()
            ->getRepository(point_schemas::class)
    ...
    protected static function pst_setter()
    {
        $pst = [];
        $temp = new \stdClass();
        $temp->p = 0;
    ...
    protected static function pst_abilities(character_template $ct, $creation)
    {
        $abilities = [
            "talents",
            "skills",
    ...
    protected static function findTraitValue($id, $traits)
    {
        $trait_count = count($traits);
        for ($i=0; $i<$trait_count; $i++) {
    ...
    protected static function findTraitByName($name, $traits)
    {
        $trait_count = count($traits);
        for($i=0; $i<$trait_count; $i++) {
    ...
    protected static function findTraitById($id, $traits)
    {
        $trait_count = count($traits);
        for($i=0; $i<$trait_count; $i++) {
            if ($traits[$i]->getId() === $id) {
